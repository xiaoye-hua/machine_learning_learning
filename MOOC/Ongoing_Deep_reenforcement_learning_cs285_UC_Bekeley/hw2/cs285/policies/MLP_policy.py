...
class MLPPolicy(BasePolicy):
    ...
    def get_action(self, obs):
        ...
        # TODO return the action that the policy prescribes --> FINISHED
...
class MLPPolicySL(MLPPolicy):
    ...
    def define_train_op(self):
        ...
        # TODO define the loss that will be used to train this policy  --> FINISHED
        # HINT1: remember that we are doing supervised learning
        # HINT2: use tf.losses.mean_squared_error
        self.loss = tf.losses.mean_squared_error(labels=true_actions, predictions=predicted_actions)
...
class MLPPolicyPG(MLPPolicy):
    ...
    def define_train_op(self):
        ...
        # # cross entropy
        # # TODO: 为啥没有 tf.one_hot?
        # self.loss = tf.reduce_sum(- tf.multiply(self.logprob_n, tf.cast(self.actions_pl, tf.float32)))
        #
        # # TODO: define what exactly the optimizer should minimize when updating the policy
        # self.train_op = tf.train.AdamOptimizer(self.learning_rate).minimize(tf.reduce_mean(self.loss*self.adv_n))
