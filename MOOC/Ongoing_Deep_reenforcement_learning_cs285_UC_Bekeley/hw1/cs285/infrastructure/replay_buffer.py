...
class ReplayBuffer(object):
    ...
    def sample_random_data(self, batch_size):
        ...
        ## TODO return batch_size number of random entries from each of the 5 component arrays above  --> FINISHED
        ## HINT 1: use np.random.permutation to sample random indices
        ## HINT 2: return corresponding data points from each array (i.e., not different indices from each array)
        ## HINT 3: look at the sample_recent_data function below
        indexs = np.random.permutation(list(range(self.obs.shape[0])))[-batch_size:]
        return self.obs[indexs], self.acs[indexs], self.rews[indexs], self.next_obs[indexs], self.terminals[indexs]
