...
def inference_network(x, input_dim, lstm_units, lstm2_units, latent_dim, hidden_size):
  ...
  """
  X_input = tf.keras.layers.Input(shape=(None, input_dim))(x)
  ...
  mu = tf.keras.layers.Dense(units=latent_dim, activation='relu')(lstm)
  sigma = tf.keras.layers.Dense(units=latent_dim, activation='relu')(lstm)
  return mu, sigma
...
def generative_network(z, max_len, hidden_size):
  ...
  """
  X_input = tf.keras.layers.Input(shape=(None, max_len))
